...
CREATE TABLE IF NOT EXISTS `bancodb`.`clientes_empresa` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_empresa_clientes_idx` (`id_empresa` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_empresa_clientes`
    ...
    REFERENCES `bancodb`.`empresa` (`id_empresa`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `bancodb`.`rolusuario` (
  ...
  CONSTRAINT `fk_empresa_rol`
    ...
    REFERENCES `bancodb`.`empresa` (`cliente`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_idrol`
    FOREIGN KEY (`idrol`)
